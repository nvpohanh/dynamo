...
| [Request Replay Tracing](request-tracing.md) | Per-request trace capture for direct DynoSim replay and optional payload logging | `DYN_REQUEST_TRACE`, `DYN_REQUEST_TRACE_DESTINATIONS`, `DYN_REQUEST_TRACE_FILE_PATH`, `DYN_REQUEST_TRACE_FORCE_LOGGING` |
...
| [Request Payload Logging](logging.md#request-payload-logging-otlp) | Per-request chat-completion payload rows exported over OTLP logs | `DYN_REQUEST_TRACE_DESTINATIONS`, `DYN_REQUEST_TRACE_FORCE_LOGGING`, `DYN_REQUEST_TRACE_OTEL_MAX_PAYLOAD_BYTES`, `OTEL_EXPORTER_OTLP_LOGS_ENDPOINT`†, `OTEL_EXPORTER_OTLP_LOGS_PROTOCOL`† |
